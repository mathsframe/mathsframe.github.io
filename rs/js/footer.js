...
// Lấy dữ liệu games từ games-data.js
const allGames = gamesData.getAll();

// Hàm xử lý live search
function liveSearch(query) {
  // Lấy element dropdown
  const dropdown = document.getElementById("searchDropdown");

  // Nếu không có từ khóa tìm kiếm, ẩn dropdown
  if (!query.trim()) {
    dropdown.style.display = "none";
    return;
  }

  // Tìm kiếm trong danh sách games
  ...
  // Giới hạn số kết quả hiển thị (tối đa 5 kết quả)
  const limitedResults = results.slice(0, 5);

  // Tạo HTML cho các kết quả và hiển thị trong dropdown
  dropdown.innerHTML = limitedResults
    .map(
      (game) => `
        <a href="${game.url}" class="search-result-item">
            ...
            ${game.isHot ? '<span class="hot-tag">HOT</span>' : ""}
        </a>
    `
    )
    .join("");
...
document.addEventListener("click", function (event) {
  ...
  // Nếu click bên ngoài search box, đóng dropdown
...
// Thêm debounce để tránh search quá nhiều lần
...
// Áp dụng debounce cho hàm liveSearch
